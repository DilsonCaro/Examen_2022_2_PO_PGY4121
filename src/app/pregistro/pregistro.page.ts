...
export class PregistroPage  {
  ...
  async agregar(rut:HTMLInputElement, nombre:HTMLInputElement, apellido:HTMLInputElement, correo:HTMLInputElement, password:HTMLInputElement, cod:HTMLInputElement)
    {
          //validar
    ...
    else if(cod.value.trim().length == 0)
    {
      const toast = await this.toast.create({
        message: 'El codigo no fue especificado',
        duration: 2000,
        color : "danger",
        position: "middle"
      });
